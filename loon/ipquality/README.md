...
| 测试运营商地区门户 | 关闭 | 实验访问北京、上海、广东运营商品牌门户，仅表示 HTTPS 可达性和总耗时 |
...
| 显示媒体与 AI 结果 | 开启 | 控制报告分区；若地区一致性仍开启，媒体请求仍会执行 |
...
- **运营商地区门户 HTTP（实验）**：并发访问北京、上海、广东运营商品牌门户。域名可能使用 CDN、重定向或反爬，不保证服务器位于标称地区；单站失败不代表运营商线路失败。
