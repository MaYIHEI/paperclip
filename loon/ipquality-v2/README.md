...
> 🧪 V2 测试版 · Loon 3.5.1(979)+

“常用单项原生快捷结果 + 多项自选网页报告”的独立 V2 版本。它与现有 `loon/ipquality` 同时存在，不覆盖、不替换正式版。

## 使用

导入 V2 测试插件：

`https://raw.githubusercontent.com/MaYIHEI/paperclip/refs/heads/main/loon/ipquality-v2/ipquality-v2.lpx?v=v2-test1`

节点或策略组页面提供四个带 `IP V2` 标识的入口：基础信息、风险、流媒体与 AI 三项快捷检测直接显示 Loon 原生结果；“IP V2 · 自选网页”通过通知进入 Safari，可选择一项或多项。
...
这是独立的 V2 测试版。除 HTTPS 轻量测速外，其余模块暂时独立执行一次 r32，因此同时开启很多项目会重复请求基础来源；后续会根据真机反馈继续拆分共享数据和模块专用探测，减少重复请求与 API 限流。
