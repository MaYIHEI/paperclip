...
**待办(下一步)**:
1. ~~抓 Cookie~~ ✅ 已跑通(`normalizeCookie` 修多行 cookie 头)
2. ~~验证刷新链路~~ ✅ 已跑通(间隔 40 分钟两次 cron 均刷新+打卡成功)
3. **观察**:① 明天未签时确认拿到 `✅ 签到成功`(目前只测到 `12904 已打卡`)② teldb 滚动跨多日是否稳定续期。注意别在脚本周期内手动开小程序签到页(会把 teldb 滚走、作废脚本存的;真开了就重抓)
