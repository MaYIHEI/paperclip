...
脚本名称：微信小程序 海底捞 签到
脚本作者：@Sliverkiss
脚本修改：@MaYIHEI (https://github.com/MaYIHEI/paperclip)
Telegram 频道：https://t.me/mayihei
更新日期：2026-05-13

【更新说明 2026-05-13】
原作脚本的接口本身仍在使用（仍是 superapp-public.kiwa-tech.com/activity/wxapp/signin/...），
失效原因是抓 token 的重写规则不对：原脚本抓的是 Authorization 头，
但实际签到鉴权头是 _haidilao_app_token（值形如 TOKEN_APP_xxx-xxx）。
- 修复 getCookie：读取 _haidilao_app_token 头
- 重写匹配路径放宽：只要命中 /activity/wxapp/signin/ 下的接口即可抓 token
- 修正函数命名混乱（保留旧名兼容）：signin 走签到接口，queryFragment 查碎片
- 适配重写规则到 paperclip raw URL

【抓 token】打开海底捞小程序 → 我的 → 任意签到入口（首次会触发 /activity/wxapp/signin/query 等接口），
即可抓到 token 并入库。

[Script]
http-request ^https:\/\/superapp-public\.kiwa-tech\.com\/activity\/wxapp\/signin\/(query|querySite|querySwitch|queryFragment|signin) script-path=https://raw.githubusercontent.com/MaYIHEI/paperclip/main/miniprogram/haidilao/haidilao.js, requires-body=false, timeout=60, tag=海底捞 Cookie
...
[Script]
海底捞 Cookie = type=http-request,pattern=^https:\/\/superapp-public\.kiwa-tech\.com\/activity\/wxapp\/signin\/(query|querySite|querySwitch|queryFragment|signin),requires-body=0,max-size=0,script-path=https://raw.githubusercontent.com/MaYIHEI/paperclip/main/miniprogram/haidilao/haidilao.js
...
[Script]
http-request ^https:\/\/superapp-public\.kiwa-tech\.com\/activity\/wxapp\/signin\/(query|querySite|querySwitch|queryFragment|signin) tag=海底捞 Cookie, script-path=https://raw.githubusercontent.com/MaYIHEI/paperclip/main/miniprogram/haidilao/haidilao.js,requires-body=0
...
[task_local]
23 7 * * * https://raw.githubusercontent.com/MaYIHEI/paperclip/main/miniprogram/haidilao/haidilao.js, tag=海底捞签到, enabled=true
