...
- **QQ 音乐内广告**:已接入固定金额“看视频领金币”、服务端后续返回的 ECPM/固定金额二选一任务,以及“看广告领金币抽奖次数”。脚本会获取对应广告位的新票据、等待 `reward_time` 后再领奖,并继续执行返回的下一条广告。固定视频链新增 `AdRewardType=0` 和按素材 `reward_gold` 自动选择 ECPM/固定任务;该分支已有 HAR 成功参数对照,仍需首次 cron 真跑确认服务端是否还校验 SDK 播放事件。
