...
async function runMicoinTasks(refRole) {
    try {
        ...
        // 3. 浏览 3 个帖子 (用前 3 个 post_id)
        await micoinViewPosts(posts.slice(0, 3), refRole);
        await sleep(1500);

        // 4. 点赞 5 个帖子 (用前 5 个 post_id,点完立即取消)
    ...
    return new Promise((resolve) => {
        ...
        $.post(opts, (err, resp, data) => {
            ...
            try {
                const r = JSON.parse(data);
                if (r.retcode !== 0) {
                    // 已签到的 retcode 可能是 1008/-1002/-1004 等,统一识别
...
// 拉最近帖子列表 GET /post/api/getRecentForumPostList
function fetchRecentPostList(forumId) {
    return new Promise((resolve) => {
        const h = cleanHeaders($.bbsHeaders);
        const opts = {
            url: `https://bbs-api.miyoushe.com/post/api/getRecentForumPostList?forum_id=${forumId}&page_size=20`,
            headers: h,
        };
        $.get(opts, (err, resp, data) => {
            if (err || !resp || resp.statusCode !== 200) {
