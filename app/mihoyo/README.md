...
```ini
[Script]
http-request https?:\/\/api-takumi(\.miyoushe|\.mihoyo)\.com\/(binding\/api\/getUserGameRolesByStoken|event\/luna\/[a-z0-9]+\/(info|home|sign))|https?:\/\/bbs-api\.miyoushe\.com\/apihub\/sapi\/getUserMissionsState tag=米游社cookie, script-path=https://raw.githubusercontent.com/MaYIHEI/paperclip/main/app/mihoyo/mihoyo.cookie.js, requires-body=0, enable=true
...
```ini
[Script]
米游社cookie# = type=http-request, pattern=https?:\/\/api-takumi(\.miyoushe|\.mihoyo)\.com\/(binding\/api\/getUserGameRolesByStoken|event\/luna\/[a-z0-9]+\/(info|home|sign))|https?:\/\/bbs-api\.miyoushe\.com\/apihub\/sapi\/getUserMissionsState, requires-body=0, script-path=https://raw.githubusercontent.com/MaYIHEI/paperclip/main/app/mihoyo/mihoyo.cookie.js
...
1. 启用上面的 cookie 重写规则
2. 打开米游社 APP,进入「我的」页面 → 抓到 stoken cookie(通知会更新进度)
3. 进任一游戏签到页面(原神/星穹铁道/绝区零都行),手动签到一次 → 抓到 web 签到 cookie
4. 在米游社 APP 进入「我的」→「米游币」页面 → 抓到 BBS headers (做米游币任务用)
5. 三条 cookie 都就绪后,关闭 cookie 重写规则(避免后续被反复覆盖)
6. cron 自动执行,默认每天 07:30

> 如果只想签到不要米游币任务,跳过步骤 4,把 `mhy_enable_micoin` 设为 `false`
