/*
 * 网上国网(SGCC/95598)积分签到 —— 复用信封免公钥方案
 * 原理:复用真实"提交签到"信封 skey+data(服务器私钥解密,与会话无关,永久有效),
 *       只重算 sign=SM3(skey+data+timestamp) + 配当前有效 t。
 *   端点 m1/0103514 = 真正"提交签到"(一天只调一次);signInConfig 仅状态轮询。
 ...
 * @Author MaYIHEI  @Updated 2026-06-17  状态:🧪 待验证(t 寿命 + 签到生效)
 */
...
function notify(t,s,b){ if(typeof $notification!=="undefined") $notification.post(t,s,b||""); console.log(t+" "+s); }

try {
  var rawH = read(KEY_HDR), rawE = read(KEY_ENV);
  if(!rawH){ notify("⚠️ 网上国网签到","没有 t,请开 app 进积分签到页抓取",""); $done(); }
  else if(!rawE){ notify("⚠️ 网上国网签到","没有签到信封,请开 app 进积分签到页(会自动抓)",""); $done(); }
  else {
    var hdr = JSON.parse(rawH), env0 = JSON.parse(rawE);
    if(!hdr.t || !hdr.userid){ notify("⚠️ 网上国网签到","t/userid 缺失,重新抓",""); $done(); }
    else if(!env0.data || !env0.skey){ notify("⚠️ 网上国网签到","签到信封缺失,重抓",""); $done(); }
    else {
      var ts = "" + Date.now();
      var sign = sm3(env0.skey + env0.data + ts);
      var body = { data: env0.data, sign: sign, skey: env0.skey, timestamp: ts };
      var headers = { "content-type":"application/json", "accept":"application/json;charset=UTF-8" };
      var keys = ["t","userid","device_token","devicetokentx","devicetokentxtime","appguid","appguidnew","wtoken","appcode","os","version","ip","province","language","wsgwtype","accessmethod","user-agent"];
      for (var i=0;i<keys.length;i++){ if(hdr[keys[i]]!=null) headers[keys[i]]=hdr[keys[i]]; }
      var path = env0.path || "/osg-omgmt1042/member/m1/0103514";
      $httpClient.post({ url:"https://csc-service.sgcc.com.cn:28630"+path, headers:headers, body:JSON.stringify(body) }, function(err,resp,data){
        if(err){ notify("⚠️ 网上国网签到","网络错误", String(err).slice(0,80)); $done(); return; }
        var msg=String(data||"").slice(0,140), ok=false;
        try{ var j=JSON.parse(data); ok=!!j.encryptData; if(!ok) msg=(j.message||"")+"["+(j.code||"")+"]"; }catch(e){}
        if(ok) notify("✅ 网上国网签到","已提交(服务器接受)","响应已加密,请到 app 积分页核对是否+分");
        else if(/RK1003|系统正忙|S101[0-9]|登录|token|过期|失效/.test(msg)) notify("⚠️ 网上国网签到","t 可能失效,重新抓", msg);
        else notify("❓ 网上国网签到","返回异常", msg);
        $done();
      });
    }
  }
} catch(e) { notify("⚠️ 网上国网签到","异常", e.message||String(e)); $done(); }
