...
try {
  ...
  if(!rawH || !rawE){ notify("⚠️ 网上国网签到","缺少 t 或签到信封","开 app 进积分签到页抓取(收到两条通知即可)"); done(); }
  else {
    var hdr = JSON.parse(rawH), env0 = JSON.parse(rawE);
    if(!hdr.t || !hdr.userid || !env0.data || !env0.skey){ notify("⚠️ 网上国网签到","t/信封字段缺失,请开 app 重抓",""); done(); }
    else {
      var path = env0.path || "/osg-omgmt1042/member/m1/0103514";
      var baseHeaders = { "content-type":"application/json", "accept":"application/json;charset=UTF-8" };
      ...
      for (var i=0;i<keys.length;i++){ if(hdr[keys[i]]!=null) baseHeaders[keys[i]]=hdr[keys[i]]; }
...
} catch(e) { notify("⚠️ 网上国网签到","脚本异常", e.message||String(e)); done(); }
