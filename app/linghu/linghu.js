 ...
 * 灵狐灵购 · 每日签到领红包,支持连签奖励
 *
 * 抓取:打开「灵狐灵购」APP →「商城」或「我的」页停留 1 秒(自动触发 my/info),抓 Cookie
 * 签到:cron 定时自动签到
 *
...
const CK_KEY = 'linghu_data';
const BASE   = 'https://cn-yx-wxmall.dreame.tech/main/member-center';

// 签名盐:body 字段排序后作为 security_key 字段参与 md5(详见本地 project.md)
const SALT = 'm_r4x!fY9dG5kp';
// 客户端固定标识(所有用户一致,非隐私;随 app 版本走,失效就重抓一次包对照)
const CLIENT = {
    'client-id':      'dreame_app_yx',
    'store-id':       'dreame_app_yx',
    'dreame-api':     'yx_i_1757554865',
    'dreame-version': '2.1.6',
};
const SDK_VERSION  = '22';
const VERSION_CODE = '1220';
const SIGN_TYPE    = '5'; // 签到活动类型,status / sign-in 都带 type=5
...
if (typeof $request !== "undefined") {
    ...
    (async () => {
        if (JSON.parse($.getdata("linghu_clear") || "false")) {
            $.setdata("", CK_KEY);
            $.setdata("false", "linghu_clear");
...
// ─── 抓 Cookie ───────────────────────────────────────────────────────────────
// 触发条件: 打开灵狐灵购 APP →「商城」或「我的」, my/info 请求 body 里带 sessid + user_id
function getCookie() {
...
async function main() {
    ...
    // 3. 拼通知:今日红包 + 连签天数(查询失败不影响签到主结果)
    const redEnvelope = r.data && r.data.redEnvelope != null
        ? `红包 +${r.data.redEnvelope}` : '';
    let continueDays = '';
    try {
        const st2 = await call('sign-in-status', ck);
        if (st2 && st2.iRet === 1 && st2.data && st2.data.continueSignDays != null) {
            continueDays = `连签 ${st2.data.continueSignDays} 天`;
        }
    } catch (e) {
        debug('查询连签天数异常,忽略: ' + e);
    }

    const body = [redEnvelope, continueDays].filter(Boolean).join(' · ') || '签到成功';
    $.msg($.name, '✅ 灵狐灵购签到成功', body);
}
...
// 调试日志:BoxJS 设 linghu_debug=true 才打印接口原始响应
function debug(content) {
    if (($.getdata("linghu_debug") || "false") !== "true") return;
